...
import { RiAddLine, RiArrowRightLine, RiArrowRightSLine, RiDownloadLine, RiInbox2Line, RiInformationLine, RiMailAddLine, RiMailLine, RiMailSendLine, RiSearch2Line, RiSearchLine, RiUploadCloud2Line } from "@remixicon/react";
...
import { Inbox, useInbox } from './InboxProvider';
...
export const AddBoxContext = createContext({
    setAdd: (value: boolean) => {},
});

export const AddBoxProvider = ({ children, func }: { children: React.ReactNode, func: () => Promise<void> }) => {
    ...
    const GetSimpleEmail = (): EmailEntry => {
    ...
    const SubmitCustom = async () => {
        ...
        setLoading(true)
        try {
            await Call("/emails/other", "POST", GetSimpleEmail())
            inbox.GetAddresses("", 0);
            inbox.setSearch("")
            setAdd(false)
        } catch (err) {
    ...
    return (
        <AddBoxContext.Provider value={{setAdd}}>
            {children}
            <div className={`fixed inset-0 z-100 bg-slate-950/45 flex justify-center items-center transition ${add ? "opacity-100 visible":"opacity-0 invisible"}`}>
                <Popup value={add}>
                    <BackTop onClick={() => setAdd(false)}>Add New Email Address</BackTop>
                ...
                <Popup value={provider === "google"}>
                    <BackTop onClick={() => setProvider("")}>Gmail / G-Suite Email Setup</BackTop>
                    <div className="grid md:grid-cols-2 gap-10 my-20">
                        <div className="grid gap-2">
                            <Step step={1}>Go to your <DefaultHref href="https://admin.google.com/u/1/ac/owl/list?tab=configuredApps">Google Workspace Admin Panel</DefaultHref>.</Step>
                            <Step step={2}>Click "Configure new app".</Step>
                            <Step step={3}>
                            ...
                            <Step step={4}>Allow Warmbly to access your Google Workspace.</Step>
                        </div>
                        <div>
                ...
                <Popup value={provider === "microsoft"}>
                    <BackTop onClick={() => setProvider("")}>Outlook / Office 365 Email Setup</BackTop>
                    <div className="grid md:grid-cols-2 gap-10 my-20">
                        <div className="grid gap-2">
                            <Step step={1}>Enable SMTP access for your Microsoft account.</Step>
                            <Step step={2}>
                ...
                <Popup value={provider === "custom"}>
                    <BackTop onClick={() => setProvider("")}>Other Email Setup (SMTP/IMAP)</BackTop>
                    <div className="max-w-5xl mx-auto overflow-x-hidden flex">
                        <div className={`flex flex-col md:flex-row w-full px-1 shrink-0 gap-10 mx-auto my-10 transition-transform ${step !== 0 || rows.length > 0 ? "-translate-x-[100%]":"translate-x-0"}`}>
                            <div className="md:grow">
                                ...
                                <div className="grid grid-cols-2 gap-3 mt-5">
                                    <div></div>
                                    <StepButton next loading={loading} onClick={() => {
                                        setStep(1)
                                    }}/>
                                </div>
                            </div>
                            ...
                            <div className="max-w-2xl flex w-full overflow-x-hidden mx-auto">
                                ...
                                <div className="w-full px-1 shrink-0 transition-transform" style={{transform: `translateX(-${(step === 2 ? 1:0)*100}%)`}}>
                                    ...
                                    <div className="grid grid-cols-2 gap-3 mt-5">
                                        ...
                                        <StepButton next loading={loading} onClick={SubmitCustom}/>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div  className={`flex flex-col w-full px-1 shrink-0 gap-10 mx-auto my-10 transition-transform ${rows.length === 0 ? "translate-x-[0]":"-translate-x-[200%]"}`}>
                            ...
                            <div className="w-full px-1 shrink-0 transition-transform" style={{transform: `translateX(-${(step-1)*100}%)`}}>
                                ...
                                <div className="grid grid-cols-2 gap-3 mt-5">
                                    <StepButton tt="Cancel" onClick={() => {
                                        setRows([])
                                    }}/>
                                    <StepButton tt='Submit' loading={loading} onClick={SubmitCustom}/>
                                </div>
                            </div>
                        </div>
                    </div>
...
export function useAddBox() {
  return useContext(AddBoxContext);
}