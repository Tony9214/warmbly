...
- signed click links: the Go sender appends `&s=<hex HMAC-SHA256(taskID|url)>` to every tracked link (`internal/tasks/template.go`, secret injected via `tasks.SetTrackingLinkSecret` from the required `EmailConfig.TrackingLinkSecret`); the tracking service verifies with the same `TRACKING_LINK_SECRET` and refuses unsigned/mis-signed redirects with `404`. This closes the open-redirector hole. There is NO unsigned mode: both binaries fail to boot without the secret, and the values must match (the dev Makefile pre-wires `local-dev-tracking-link-secret` on both)
- key rotation is revocation: there is exactly one valid secret and no grace window. Rotating immediately 404s links in already-delivered emails — deliberate, because a rotated key is an untrusted key. Only rotate on compromise; do not add a previous-secret fallback
