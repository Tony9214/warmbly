...
func (r *emailRepository) Search(ctx context.Context, userID, search string, cursor, tag *string, limit int32) (*models.EmailsResult, *errx.Error) {
	...
	query := `
		SELECT 
		 ...
		 ea.warmup_max, ea.warmup_increase, ea.created_at, ea.updated_at,
		 COALESCE(
...
func (r *emailRepository) Update(ctx context.Context, userID, emailAccountID string, udata *models.UpdateEmail) (*models.Email, *errx.Error) {
	...
	query := fmt.Sprintf(`
		...
		RETURNING id, email, name, signature_plain, signature_html, signature_sync, signature_code, provider, status,
		          ...
		          warmup, warmup_base, warmup_max, warmup_increase, warmup_reply_rate, warmup_tag, created_at, updated_at
