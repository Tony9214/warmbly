...
// GET /campaigns/:id/leads/order
func (h *Handler) ListCampaignLeadOrder(c *gin.Context) {
	orgID := middleware.GetOrganizationID(c)
	if orgID == nil {
		errx.JSON(c, errx.New(errx.BadRequest, "no organization selected"))
		return
	...
	leads, xerr := h.CampaignService.ListCampaignLeadOrder(c.Request.Context(), *orgID, c.Param("id"), limit)
	if xerr != nil {
		errx.JSON(c, xerr)
		return
	}
...
// ReplaceCampaignLeadOrder persists a manual send order for the campaign.
// PUT /campaigns/:id/leads/order  body: { "contact_ids": ["uuid", ...] }
func (h *Handler) ReplaceCampaignLeadOrder(c *gin.Context) {
	orgID := middleware.GetOrganizationID(c)
	if orgID == nil {
		errx.JSON(c, errx.New(errx.BadRequest, "no organization selected"))
		return
	...
	ids := make([]uuid.UUID, 0, len(body.ContactIDs))
	for _, s := range body.ContactIDs {
		id, err := uuid.Parse(s)
		if err != nil {
	...
	leads, xerr := h.CampaignService.ReplaceCampaignLeadOrder(c.Request.Context(), *orgID, c.Param("id"), ids)
	if xerr != nil {
		errx.JSON(c, xerr)
		return
	}
	...
	if campaignID, err := uuid.Parse(c.Param("id")); err == nil {
		h.auditOrg(c, models.AuditActionUpdate, models.AuditEntityCampaign, &campaignID, nil, map[string]string{"scope": "lead_order"})
	}
