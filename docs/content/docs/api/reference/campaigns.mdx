...
```json
{
  ...
  "pagination": {
    "total": 12,
    "next_cursor": "8f1d6b2e-2b7a-4c9e-9a1f-0e6d4c3b2a10",
    "has_more": true
  }
}
```
...
List the campaign's A/B variants. A variant scoped to a `sequence_id` applies to one step, a `null` sequence id is campaign-level. **Scope** `READ_CAMPAIGNS` · **Org permission** `view_campaigns`.
...
Add a variant to the campaign (or to one step via `sequence_id`). **Scope** `WRITE_CAMPAIGNS` · **Org permission** `manage_settings`.
...
| `sequence_id` | uuid | no | Step to scope the variant to, omit for campaign-level. |
...
| `sequence_id` | form (multipart) | uuid | Optional. Scope the attachment to one sequence step. |
...
Send a one-off preview of a sequence step to a chosen recipient through a chosen mailbox. Defaults to the first step when `sequence_id` is omitted. **Scope** `SEND_CAMPAIGNS` · **Org permission** `send_campaigns`.
...
| `sequence_id` | uuid | no | Step to render and send, defaults to the first step. |
...
`PATCH /campaigns/:id/sequences/:sid`
...
`DELETE /campaigns/:id/sequences/:sid`
