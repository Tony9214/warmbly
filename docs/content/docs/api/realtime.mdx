...
The dashboard streams ephemeral collaboration frames so teammates see each other in real time: live cursors everywhere, plus in-flight card drags on the builder canvases. These ride a separate fast path from the sequenced event stream above.
...
- `automation:<id>` and `campaign:<id>` for the automation and campaign-sequence builder canvases (cursors and card drags). Coordinates are canvas (flow) space, so a point lands in the same place for everyone regardless of pan or zoom.
...
Ephemeral [live-collaboration](#live-collaboration) frames (`live:cursor`, `live:node`) are exempt from the client-event budget above: they are bounded by a separate small per-socket burst limit, and over-budget frames are dropped silently rather than rejected.
