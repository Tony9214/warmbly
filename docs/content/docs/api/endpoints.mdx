...
Registering and managing the OAuth 2.1 apps your workspace owns. The flow itself (authorize, token, revoke) is listed under JWT only and Public below.
...
- `POST /oauth/token`, `POST /oauth/revoke` (OAuth 2.1 token endpoints, authenticated by the client's id and secret)
