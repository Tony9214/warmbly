...
The **Leads** tab shows one status per contact so you can see where each lead stands in the sequence at a glance:

- **Queued**: a lead in the campaign that has not been emailed yet.
- **Processing**: some, but not all, sequence steps have been sent. The lead is still moving through the flow.
- **Done**: every step in the sequence has been sent and the lead has not replied or bounced. There is nothing left to send this contact.
- **Replied**: the contact replied. Sending to them stops.
- **Bounced**: a send hard-bounced.
- **Unsubscribed**: the contact unsubscribed or is suppressed.

A lead stays **Processing** only while steps remain to send. Once the last step goes out it moves to **Done**, so a finished campaign reads as done rather than stuck mid-flight.
